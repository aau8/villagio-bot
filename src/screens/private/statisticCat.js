...
const sendStatCatPrivate = async (ctx) => {
	...
	return send(ctx, $i18n(ctx, `private.${commandText}`, { lng: 'ru' }), {
		reply_markup: {
			inline_keyboard: [
				[
					{ text: $i18n(ctx, 'private.kb.download_json', { lng: 'ru' }), url: `${API_URL}/stats?cat=${command}&format=json&admin_id=${user.id}` },
					{ text: $i18n(ctx, 'private.kb.download_xlsx', { lng: 'ru' }), url: `${API_URL}/stats?cat=${command}&format=xlsx&admin_id=${user.id}` },
				],
				[ { text: $i18n(ctx, 'kb.back', { lng: 'ru' }), callback_data: "statistic" }, ],
			]
		}
	})
}
